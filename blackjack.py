...
class Player:
    ...
    def place_bet(self):
        while True:
            try:
                bet = int(
                    input(f"You have ${self.money}. How much would you like to bet? ")
                )
                if 0 < bet <= self.money:
                    self.money -= bet
                    return bet
...
def play(deck, players_count, turn, dealers_count, bet):
    ...
    if choice == "h":
        ...
        print("Dealer's cards are: " + deck[1].print_card() + ", hidden")
        print(
            "Your cards are: " + deck[0].print_card() + ", " + deck[2].print_card(),
            end="",
        )
        for i in range(turn):
...
def dealers_turn(deck, dealers_count, players_count, card_index, bet):
    ...
    while dealers_count <= 21:  # Ensure we don't run out of cards in the deck
